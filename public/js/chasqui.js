...
function  verinfo_usuario(id, form){
  ...
  if(form == 2){var miurl =urlraiz+"/form_baja_usuario/"+id+""; }
...
//Funcion para generar la ventana emergente de los formularios
function mostrar_formulario(arg){
   var urlraiz=$("#url_raiz_proyecto").val();
   $("#capa_modal").show();
   $("#capa_formularios").show();
   var screenTop = $(document).scrollTop();
   $("#capa_formularios").css('top', screenTop);
   $("#capa_formularios").html($("#cargador_empresa").html());
   if(arg==1){ var miurl=urlraiz+"/form_nueva_correspondencia"; }
...
//Funcion para generar la ventana emergente del formulario responder o derivar correspondencia
...
$(document).on("submit",".formentrada",function(e){
  ...
  var varurl="";
    ...
    if(quien=="f_nueva_correspondencia"){  var varurl=$(this).attr("action");  var div_resul="div_notificacion_sol";}
  ...
  if(quien=="f_verificar_solicitud"){  var varurl=$(this).attr("action");  var div_resul="capa_formularios";  }
    ...
    success : function(resul) {
      ...
      if(quien=="f_agregar_fechas"){
        if (resul == 'ok') {
          ...
          alertify.warning('Está tratando de ingresar una fecha anterior!');
          $('#btn_guarda_fecha').attr("disabled", false);
        }
