		  ...
		  <table class="table table-hover">
			...
			@foreach ($personal as $p)
				...
				<tr>
					...
					<td>
						<button type="button" class="btn  btn-success btn-xs" disabled onclick="aceptar_suspension_rr_hh({{  $p->id_suspension }})" ><i class="fa fa-fw fa-check"></i></button>
						<button type="button" class="btn  btn-default btn-xs" disabled onclick="verinfo_usuario({{  $p->id_suspension }}, 11)" ><i class="fa fa-fw fa-eye"></i></button>
					</td>
					@endif
					@if ($p->estado == 'AUTORIZADA')
					<td><span class="badge bg-yellow">{{$p->estado}}</span></td>
					<td>
						<button type="button" class="btn  btn-success btn-xs" onclick="aceptar_suspension_rr_hh({{  $p->id_suspension }})" ><i class="fa fa-fw fa-check"></i></button>
						<button type="button" class="btn  btn-default btn-xs" onclick="verinfo_usuario({{  $p->id_suspension }}, 11)" ><i class="fa fa-fw fa-eye"></i></button>
					</td>
					@endif
					@if ($p->estado == 'VERIFICADA')
